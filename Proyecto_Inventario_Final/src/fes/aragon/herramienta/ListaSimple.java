...
public class ListaSimple<E> {
	...
	public void agregarEnCabeza(E dato) {
		cabeza = new Nodo<E>(dato, cabeza, null);
		if (cola == null) {
			cola = cabeza;
		}
		longitud++;
	 ...
	 * @return retorna la posicion del nodo del dato a buscar, retorna -1 si no lo encuentra
	 */
		...
		if (indice >= 0 && indice <= longitud - 1) {
			if (indice == 0) {
				...
				for (prv = null, tmp = cabeza; contador < indice; contador++, prv = tmp, tmp = tmp.getSiguiente())
					;
				prv.setSiguiente(new Nodo<E>(dato, tmp, null));
				longitud++;
				seinserto = true;
			}
		}
